...
public class MemberApp {
    public static void main(String[] args) {
        ...
        //AppConfig.class에서 @Bean이 붙은 각 메소드의 반환값(객체)을 스프링 컨테이너에 등록하고 관리해줌
        ApplicationContext applicationContext = new AnnotationConfigApplicationContext(AppConfig.class);

        /* 스프링 컨테이너에서 빈을 꺼낼때는(getBean메소드 사용) 빈의 이름으로 꺼내야 하는데,
           기본적으로 빈의 이름은 @Bean이 붙어있는 해당 메소드의 이름으로 등록되어있음.
        //ac.getBean(빈이름, 타입)
        //ac.getBean(타입)   //타입으로 조회 시 하나일 때만 가능, 만약 둘 이상이 존재하면 에러 뜸.
          <getBean 메소드>
           1. 첫번째 인자(key): 빈 이름(==메소드 이름)
           2. 두번째 인자(value): 메소드의 반환 타입(클래스 단위)      */
